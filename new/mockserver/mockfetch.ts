...
import solBlockHeight from "../exempleresponse/sol/getBlockHeight_sol.json" with { type: "json" };
import solPrioFees from "../exempleresponse/sol/getRecentPrioritizationFees_sol.json" with { type: "json" };
import blockByNumberEth from "../exempleresponse/eth/eth_getBlockByNumber_eth.json" with { type: "json" };
import blockByNumberBase from "../exempleresponse/base/eth_getBlockByNumber_base.json" with { type: "json" };
...
const SOL_TX = rpc({
    ...
    meta: {
        ...
        // Transfert natif de 0,5 SOL : le compte 1 (destinataire) gagne 500 000 000 lamports.
        preBalances: [2_000_000_000, 0, 1],
        postBalances: [1_499_989_950, 500_000_000, 1]
    },
    transaction: {
        message: {
            accountKeys: [
...
// Estimations de frais Esplora (sat/vB) par cible de confirmation, lues par BitcoinCongestion
// via la clé "1" (prochain bloc). 8 sat/vB < seuil de 30 → réseau « fluide ».
const BTC_FEE_ESTIMATES = { "1": 8, "2": 7, "3": 6, "6": 5, "144": 2, "1008": 1 };
...
export async function mockFetch(input: any, options: any = {}): Promise<Response> {
    ...
    if (options?.method === "POST" && options.body) {
        ...
        switch (method) {
            ...
            case "eth_getBlockByNumber":
                return json(isBase ? blockByNumberBase : blockByNumberEth);

            case "getBalance":
                return json(SOL_BALANCE);
            case "getBlockHeight":
                return json(solBlockHeight);
            case "getRecentPrioritizationFees":
                return json(solPrioFees);
    ...
    if (url.endsWith("/fee-estimates")) return json(BTC_FEE_ESTIMATES);
